...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        print('\n')
        inpt = str(input('Guess a letter, phrase, or type "exit" or "pass":'))
        return inpt
        ...
        if possible:
            if self.smartCoinFlip():
                ...
                for letter in possible:
                    if SORTED_FREQUENCIES.index(letter) >= max_index:
                        max_index = SORTED_FREQUENCIES.index(letter)
                return SORTED_FREQUENCIES[max_index]
            else:
...
# Spins the wheel of fortune wheel to give a random prize Examples: {
# "type": "cash", "text": "$950", "value": 950, "prize": "A trip to Ann
# Arbor!" }, { "type": "bankrupt", "text": "Bankrupt", "prize": false },
...
# playerIndex keeps track of the index (0 to len(players)-1) of the player
# whose turn it is
playerIndex = 0
...
def requestPlayerMove(player, category, guessed):
    while True:  # we're going to keep asking the player for a move until
        # they give a valid one
        time.sleep(0.1)
        # added so that any feedback is printed out before the
        # next prompt

        move = player.getMove(category, obscurePhrase(phrase, guessed),
                              guessed)
        move = move.upper()  # convert whatever the player entered to UPPERCASE
        if move == 'EXIT' or move == 'PASS':
            return move
        elif len(move) == 1:  # they guessed a character
            if move not in LETTERS:  # the user entered an invalid letter (
                # such as @, #, or $)
                ...
                break
            elif move in VOWELS and player.prizeMoney < VOWEL_COST:  # if
                # it's a vowel, we need to be sure the player has enough
                print(
                    'Need ${} to guess a vowel. Try again.'.format(VOWEL_COST))
                continue
            else:
                return move
...
while True:
    ...
    if wheelPrize['type'] == 'bankrupt':
        ...
        if move == 'EXIT':  # leave the game
        ...
        elif move:  # they guessed a letter
            guessed.append(move)
            ...
            count = phrase.count(
                move)  # returns an integer with how many times this letter appears
            if count:
                if count == 1:
    ...
    # Move on to the next player (or go back to player[0] if the end)
